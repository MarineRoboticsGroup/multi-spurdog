...
class CycleManager:
    ...
    def __init__(self):
        ...
        s#elf.acomms_event = rospy.Publisher("acomms_event", Time, queue_size=1)
    ...
    def on_tdma_status(self, msg):
        ...
        # Load the cycle message into the queue so its ready to go when we are active
            if time_to_next_active < self.msg_preload:
    ...
    def request_preintegration(self, tj):
        ...
        try:
            preintegrate_imu = rospy.ServiceProxy('preintegrate_imu', PreintegrateIMU)
            response = preintegrate_imu(ti, tj)
