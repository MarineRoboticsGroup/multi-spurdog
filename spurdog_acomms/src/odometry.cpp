...
void ImuPreintegratorNode::imuCallback(const sensor_msgs::Imu::ConstPtr& msg) {
  ...
  if (vel_buffer_.empty()) { // A proxy for a lack of DVL or a DVL failure
    ...
    gtsam::Matrix3 constant_vel_noise_enu = R_ned_to_enu.matrix() * constant_vel_noise_model_ * R_ned_to_enu.matrix().transpose();
...
void ImuPreintegratorNode::dvlVelCallback(const geometry_msgs::TwistStamped::ConstPtr& msg) {
  ...
  gtsam::Matrix3 dvl_noise_enu = R_ned_to_enu.matrix() * dvl_noise_model_ * R_ned_to_enu.matrix().transpose();
...
std::pair<gtsam::Pose3, gtsam::Matrix6> ImuPreintegratorNode::deadReckonFromPreintegrate(
    const ros::Time& final_time, const gtsam::Pose3& Tij_body, const gtsam::Matrix6& SigmaTij_body) {
  // Get the transform at the initial time
  gtsam::Pose3 Ti_world = std::get<1>(dr_state_and_cov_); // in world ENU frame
  gtsam::Matrix6 SigmaTi_world = std::get<2>(dr_state_and_cov_);
  // Compute the Tj_world and SigmaTj_world
  gtsam::Matrix66 Ji, Jj;
  gtsam::Pose3 Tj_world = Ti_world.transformPoseFrom(Tij_body, Ji, Jj);
  gtsam::Matrix6 SigmaTj_world = Ji * SigmaTi_world * Ji.transpose() +
                             Jj * SigmaTij_body * Jj.transpose();
  // Get the last_nav_report_ and extract the orientation and depth
  gtsam::Pose3 last_nav_pose = last_nav_report_.second; // in world ENU frame
  gtsam::Rot3 last_nav_rotation = last_nav_pose.rotation(); // Get the rotation
  gtsam::Point3 last_nav_translation = last_nav_pose.translation(); // Get the translation
  double last_nav_depth = last_nav_translation.z(); // Depth is negative in ENU frame

  // Re-write the dead reckoning state to use the last_nav_report_ orientation and depth
  // This is to ensure that the dead reckoning state is consistent with the last nav report
  gtsam::Pose3 Tj_world_rewritten = gtsam::Pose3(
      last_nav_rotation, // Use the last nav report rotation
      gtsam::Point3(Tj_world.x(), Tj_world.y(), last_nav_depth)); // Use the last nav report depth
  // Update the Tj_world and SigmaTj
  ...
  dr_msg.header.frame_id = "cv7_ahrs"; // Use the same frame
  dr_msg.pose.pose.position.x = Tj_world_rewritten.x();
  dr_msg.pose.pose.position.y = Tj_world_rewritten.y();
  dr_msg.pose.pose.position.z = Tj_world_rewritten.z();
  // Get the quaternion orientation from the NavState
  gtsam::Quaternion q = Tj_world_rewritten.rotation().toQuaternion();
  dr_msg.pose.pose.orientation.w = q.w();
  dr_msg.pose.pose.orientation.x = q.x();
  dr_msg.pose.pose.orientation.y = q.y();
  dr_msg.pose.pose.orientation.z = q.z();
  ...
  for (int i = 0; i < 6; ++i) {
    for (int j = 0; j < 6; ++j) {
      ...
      dr_msg.pose.covariance[i * 6 + j] = SigmaTj_world(src_i, src_j);
    }
  }
  // Publish the dead reckoned pose
  dr_state_pub_.publish(dr_msg);
  // Update DR state and covariance
  dr_state_and_cov_ = std::make_tuple(final_time, Tj_world, SigmaTj_world);
  dead_reckon_map_[final_time] = Tj_world;
  return std::make_pair(Tj_world, SigmaTj_world);
}
...
bool ImuPreintegratorNode::handlePreintegrate(
  ...
  auto dr_result = deadReckonFromPreintegrate(req.final_time, Tij_body, SigmaTij_body);
