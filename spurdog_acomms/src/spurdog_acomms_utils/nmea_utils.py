...
def parse_nmea_carfp(nmea_data: list):
    """ Modem-to-host acknowledgement that a minipacket has been recieved
        $CARFP,TIME,<>,SRC,DEST,RATE,<>,<>,<>,Data,<>,<>*CS
        ...
        Successful recieved ping generates:
        $CACMA,PNG,2025-07-01T02:39:54.631989,1,3,31.5,9.34,1,0,0*7E
        $CARFP,2025-07-01T02:39:55.437363,0,1,3,1,-1,0,0,1;5;08010306ff,1;2;4232,*0E
    """
    # Get the fields
    if len(nmea_data) != 11:
        ...
        if nmea_data[5] == "-1": # $CARFP data[5]=-1 indicates a ping minipacket
            if len(nmea_data) == 9:
                data_payload = ""
                num_frames = 0
            elif len(nmea_data) == 10:
                hex_data = nmea_data[9]
                num_frames = nmea_data[9].split(";")[0] # Splits the data into frames, then takes the first frame (avoiding header data)
