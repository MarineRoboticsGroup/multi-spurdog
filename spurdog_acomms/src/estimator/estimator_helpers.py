...
def get_diag_relpose_covar(diags: ndarray) -> Union[RelPoseCovar6, RelPoseCovar3]:
    """Creates a RelPoseCovar6 or RelPoseCovar3 object from the diagonal
    elements of the covariance matrix.

    Args:
        diags (ndarray): the diagonal elements of the covariance matrix

    Returns:
        Union[RelPoseCovar6, RelPoseCovar3]: the corresponding covariance object
    """
    ...
    if length == 6:
        ...
        # check that the last 2 elements are the same
        assert np.allclose(
            diags[2:], diags[2]
        ), "Last 2 elements must be the same for RelPoseCovar3"
